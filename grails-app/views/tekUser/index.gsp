		...
		<div id="list-tekUser" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="firstName" title="${message(code: 'tekUser.firstName.label', default: 'First Name')}" />
					
						<g:sortableColumn property="lastName" title="${message(code: 'tekUser.lastName.label', default: 'Last Name')}" />
				...
				<g:each in="${tekUserInstanceList}" status="i" var="tekUserInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${tekUserInstance.id}">${fieldValue(bean: tekUserInstance, field: "firstName")}</g:link></td>
					
						<td>${fieldValue(bean: tekUserInstance, field: "lastName")}</td>
