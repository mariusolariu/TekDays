			...
			<thead>
				...
				<g:each in="${tekMessageInstanceList}" status="i" var="tekMessageInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${tekMessageInstance.id}">${fieldValue(bean: tekMessageInstance, field: "author")}</g:link></td>
						...
						<td>${fieldValue(bean: tekMessageInstance, field: "subject")}</td>
